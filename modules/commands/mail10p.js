...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (args[0] == "new") {
    ...
    return api.sendMessage(`[ Mail 10P ]\n${global.config.PREFIX}${this.config.name} new -> Tạo mail mới\n${global.config.PREFIX}${this.config.name} check -> Check hộp thư đến\n${global.config.PREFIX}${this.config.name} get -> Lấy mail hiện tại\n${global.config.PREFIX}${this.config.name} list -> Xem list mail\n${global.config.PREFIX}${this.config.name} more -> Thêm mail mới\n\nBạn có thể click vào url mail và nhập Key Mail để xem nội dung mail`, event.threadID, event.messageID)
  }
}